                                        ...
                                        {index !== 0 && (
                                            ...
                                            content={
                                                <div className="lg:w-[700px] md:w-[80%] flex gap-2 flex-wrap">
                                                    {stepLists
                                                        ?.filter((item) => item.flowStep.handler === 'MaterialActionHandler')
                                                        ?.map((el) => (
                                                            <div
                                                                onClick={() => addupStep(el, index)}
...
(当素材只有一个图片类型的字段的时候可开启此功能)"
                                            ...
                                            {index !== 0 &&
                                                index !== appData?.configuration?.appInformation?.workflowConfig?.steps?.length - 1 && (
                                                    <AccordionDetails>
                                                        <div key={item.field}>
                                                                        ...
                                                                        {el?.isShow && (
                                                                            <MarketForm
                                                                                ...
                                                                                onChange={(e: any) => {
                                                                                    const newList = _.cloneDeep(generRef.current);
                                                                                    const type =
                                                                                        e.name === 'MATERIAL_TYPE' ? e.value : undefined;
                                                                                    const code = item?.flowStep?.handler;
                                                                                    newList[index - 1].variable.variables[i].value =
                                                                                        e.value;
                                                                                    if (
                                                                                        type &&
                                                                                        item.variable.variables?.find(
